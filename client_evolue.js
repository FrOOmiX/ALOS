...
describe('app work request', () => {
  ...
  it('get user stats for ' + paulWR.applicant, (done) => {
    client.get('/api/dt/stats/' + paulWR.applicant, (err, req, res, result) => {
      if (err) return done(err);
      expect(result.success).to.be.true();
...
//Indexation test
  it('get indexed WR "PC update" with "upda"', (done) => {
    client.get('/api/dt/search/upda', (err, req, res, result) => {
      if (err) return done(err);
      expect(result.success).to.be.true();
      expect(result.data[0].id).to.be.equals(paulWR.id);
      done()
    })
  })

  it('get indexed WR false parameter', (done) => {
    client.get('/api/dt/search/updar', (err, req, res, result) => {
      if (err) return done(err);
      expect(result.success).to.be.false();
      done()
    })
